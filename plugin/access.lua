...
local function iter(config_array, template_env)
  return function(config_array, i, previous_name, previous_value)
    ...
    if err then
      return error("[request-transformer-improved] failed to render the template " ..
                   current_value .. ", error:" .. err)
    end

    kong.log.debug("[request-transformer-improved] template `", current_value,
                   "` rendered to `", res, "`")
...
local function transform_headers(conf)
  ...
  -- Add header(s)
  for key, value in pairs(conf.headers.add or {}) do
    if not headers[key] and key:lower() ~= HOST then
      headers[key] = value
    end
  end
...
local function transform_uri(conf, template_env)
  if conf.replace.uri then
    ...
    if err then
      error("[request-transformer-improved] failed to render the template " ..
        tostring(conf.replace.uri) .. ", error:" .. err)
    end

    kong.log.debug(DEBUG, "[request-transformer-improved] template `", conf.replace.uri,
      "` rendered to `", res, "`")
...
function _M.execute(conf)
  ...
  local ok, err = pcall(transform_headers, conf)
  if not ok then
      kong.log.crit(err)
  end
